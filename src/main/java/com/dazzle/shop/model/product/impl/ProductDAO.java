...
import com.dazzle.shop.model.cart.impl.Cart2RowMapper;
...
public class ProductDAO {
	...
	private final String add_cart = "INSERT INTO cart " + "(user_num, product_code, amount) " + "VALUES (?, ?, ?)";
	...
	private final String INSERT_REVIEW_IMG = "INSERT INTO review_img(review_num, review_img)" + " VALUES(?, ?);";

	private final String COMPARE_CART = "select * from cart where user_num = ? AND product_code = ?";
	...
	public ProductCodeVO get_product_code(int _size_num) {
		...
		try {
			return jdbc_template.queryForObject(get_product_code, new Object[] { _size_num },
					new ProductCodeRowMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public void insertReview(ReviewVO vo) {
		...
		jdbc_template.update(INSERT_REVIEW, vo.getUser_num(), vo.getProduct_code(), vo.getReview_content(),
				vo.getReview_ratings());
		return;
	}
	...
	public ReviewVO getReviewOne(ReviewVO vo) {
		...
		try {
			return jdbc_template.queryForObject(GET_REVIEW_ONE,
					new Object[] { vo.getUser_num(), vo.getProduct_code(), vo.getReview_ratings() },
					new ReviewNumRowMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	public CartVO compareCart(CartVO vo) {
		System.out.println("compareCart()");
		try {
			return jdbc_template.queryForObject(COMPARE_CART, new Object[] { vo.getUser_num(), vo.getProduct_code() },
					new Cart2RowMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
