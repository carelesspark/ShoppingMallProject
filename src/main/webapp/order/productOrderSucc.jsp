		...
		<div id="order-info">
			<div id="order-success">
				...
				<div>
					...
					<div>
						주문번호 :
						<%=order.getOrder_num()%></div>
					<div>
						주문날짜 :
						<%=order.getOrder_date()%></div>
				</div>
			</div>
			<br>
			<hr>
			<div>
				...
				<div class="product-list">
					<div class="image">
						<svg width="197" height="163" viewBox="0 0 197 163" fill="none"
							xmlns="http://www.w3.org/2000/svg"
					...
					<div class="product">
						...
						<div class="product-info">
							...
							<div><%=product.getSize_name()%>
								/
								<%=product.getColor_name()%>
							</div>
						</div>
						<div class="product-price">
							<div class="column">구매가격</div>
							<div><%=numberFormat.format(product.getTotal_price())%>
								원
							</div>
						</div>
						<div class="sales-volume">
							<div class="column">구매수량</div>
							<div><%=product.getAmount()%>
								개
							</div>
						</div>
						<div class="sum">
							<div class="column">합계</div>
							<div><%=numberFormat.format(product.getTotal_price() * product.getAmount())%>
								원
							</div>
						</div>
					</div>
			...
			<div>
				<div id="order-fee">
					...
					<div>
						<div>총 결재 금액</div>
						<div><%=formattedTotalPaymentAmount%>
							원
						</div>
					</div>
				</div>
